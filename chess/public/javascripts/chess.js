...
//
// Création des classes de pièces
//
class p {
    constructor(pos) {
        this.name = '';
...
jeu.addEventListener('click',function (e){
    ...
    if (typeof(Manger_possible)!="undefined" && typeof(Mouvementnonbloque)!="undefined" && pieceDuJoueur.includes(e.target.id) && typeof(PieceSelectionne)!="undefined"&& PieceSelectionne.pos!==''){
            clique1 = true
    ...
    if (clique1 && !promotion) {
        if (e.target.className == "grid-item") {
            PieceSelectionne = piece_at_this_position(e.target.id)
        ...
        DesaffichageCasemangable(Manger_possible,'')
            if (Manger_possible.includes(e.target.id)){
                Nb_sans_manger=0
                var posmang = e.target.id
                piece=piece_at_this_position(e.target.id);
                //pour la prise en passant
                if (piece==undefined) {
                    posi2=e.target.id
                    if (posi2[1] == 6) {
            ...
            else if (Mouvementnonbloque.includes(e.target.id)) {
                if (PieceSelectionne.name[1] != 'p'){
                    Nb_sans_manger++;
                }
                else {
                ...
                PieceSelectionne.destroy()
                Game.push(PieceSelectionne.pos + e.target.id)
                PieceSelectionne.pos = e.target.id
                PieceSelectionne.affichage()
            ...
            else if (PieceSelectionne.name[1] == 'k' && typeof(MouvementrockKing)!= 'undefined' && MouvementrockKing.includes(e.target.id) ){
                var rook
                Nb_sans_manger++;
                var rookposmem
                PieceSelectionne.rock = false
                var index = MouvementrockKing.indexOf(e.target.id)
                ...
                PieceSelectionne.destroy()
                Game.push(PieceSelectionne.pos + e.target.id + rookposmem + PosRook+ 'o')
                PieceSelectionne.pos = e.target.id
                PieceSelectionne.affichage()
                mouve.innerHTML=Game;
            }
...
promotionBot.addEventListener('click',function (e){
    var Piecepromo
    var choisi = false
    if (e.target.className.includes("promotion-piece")){
        if (e.target.className == "promotion-piece wb"){
            Piecepromo = new b()
            Piecepromo.name = promotion_pion.name[0]+'b'
            choisi = true
            Wpiece[3].push(Piecepromo)
        }
        if (e.target.className == "promotion-piece wq"){
            Piecepromo = new q()
            Piecepromo.name = promotion_pion.name[0]+'q'
            choisi = true
            Wpiece[4].push(Piecepromo)
        }
        if (e.target.className == "promotion-piece wn"){
            Piecepromo = new n()
            Piecepromo.name = promotion_pion.name[0]+'n'
            choisi = true
            Wpiece[2].push(Piecepromo)
        }
        if (e.target.className == "promotion-piece wr"){
            Piecepromo = new r()
...
promotionTop.addEventListener('click',function (e){
    ...
    var choisi = false
    if (e.target.className.includes("promotion-piece")){
        if (e.target.className == "promotion-piece bb"){
            Piecepromo = new b()
            Piecepromo.name = promotion_pion.name[0]+'b'
            choisi = true
            Bpiece[3].push(Piecepromo)
        }
        if (e.target.className == "promotion-piece bq"){
            Piecepromo = new q()
            Piecepromo.name = promotion_pion.name[0]+'q'
            choisi = true
            Bpiece[4].push(Piecepromo)
        }
        if (e.target.className == "promotion-piece bn"){
            Piecepromo = new n()
            Piecepromo.name = promotion_pion.name[0]+'n'
            choisi = true
            Bpiece[2].push(Piecepromo)
        }
        if (e.target.className == "promotion-piece br"){
            Piecepromo = new r()
            Piecepromo.name = promotion_pion.name[0]+'r'
            choisi = true
            Bpiece[1].push(Piecepromo)
        }
    }
    if (choisi){
        Piecepromo.pos = promotion_pion.pos
